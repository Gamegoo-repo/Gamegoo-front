...
export class PostProcessor {
  ...
  private async processApiIndex(): Promise<void> {
    ...
    if (await fs.pathExists(apiIndexPath)) {
      ...
      content = content.replace(/export \* from ['"]\.\//g, 'export * from "./')
  ...
  private async cleanupOldFiles(
    ...
    console.info(chalk.gray(`  ✓ Removed ${wrapperFiles.length} wrapper files`))
  }
  ...
  private async extractCommonEnums(): Promise<void> {
    ...
    if (commonEnums.size > 0) {
      ...
      console.info(chalk.gray(`  ✓ Extracted ${commonEnums.size} common enums`))
    }
  }
