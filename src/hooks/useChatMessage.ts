...
const useChatMessage = () => {
  ...
  const [mannerSystemMessage, setMannerSystemMessage] =
    useState<SystemMessage | null>(null);

  const currentChatUuid = useSelector(
    (state: RootState) => state.chat.currentChatUuid
  );
  const unreadChatUuids = useSelector(
    (state: RootState) => state.chat.unreadUuids
  );
  ...
  useEffect(() => {
    ...
    const handleMyMessage = (res: any) => {
      if (res.data.chatroomUuid === currentChatUuid) {
